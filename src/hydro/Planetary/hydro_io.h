...
INLINE static void convert_part_soundspeed(const struct engine* e,
                                           const struct part* p,
                                           const struct xpart* xp, float* ret) {
...
INLINE static void convert_part_c_diff(const struct engine* e,
                                       const struct part* p,
                                       const struct xpart* xp, float* ret) {

  ret[0] = hydro_get_comoving_c_diff(p);
}
...
INLINE static void hydro_write_particles(const struct part* parts,
  ...
  *num_fields = 13;
  ...
  list[12] = io_make_output_field_convert_part(
      "c_diff", FLOAT, 1, UNIT_CONV_SPEED, 0.f, parts, xparts,
      convert_part_c_diff, "sound speed differnce");
}
