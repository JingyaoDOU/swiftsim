...
INLINE static void gpart_to_grid_TSC(const struct gpart* gp, double* rho,
  ...
  const double pos_x = box_wrap(gp->x[0], 0., dim[0]) * fac;
  const double pos_y = box_wrap(gp->x[1], 0., dim[1]) * fac;
  const double pos_z = box_wrap(gp->x[2], 0., dim[2]) * fac;
...
void cell_to_powgrid_mapper(void* map_data, int num, void* extra) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    cell_to_powgrid(c, grid, Ngrid, gridfac, type, order, e, nu_model);
  }
}
...
void power_spectrum(const enum power_type type1, const enum power_type type2,
  ...
  if (e->nodeID == 0) {
    ...
    for (int j = 0; j < numtot; ++j) {
      fprintf(outputfile, "%12.7f %g %g %g\n", s->e->cosmology->z, kcomb[j],
              (pcomb[j] - shot), shot);
    }
    fclose(outputfile);
  }
